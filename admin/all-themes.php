                            ...
                            <?php 
                                ...
                                if(mysqli_num_rows($themes) > 0){
                                    foreach ($themes as $item){
                                        ?>
                                            <tr onclick="redirection('details-theme.php?id=<?= $item['id_themes'] ?>')">
                                                <td><?= $item['id_themes'] ?></td>
                                                <td><?= $item['libelle_theme'] ?></td>
                                                <td><?= $item['exposants']?></td>
                                                <td>
                                                    <img src="../uploads/<?= $item['image'] ?>" alt="<?= $item['image'] ?>" height="50px" width="auto">
                                                </td>
                                                <td>
                                                    <a href="edit-theme.php?id=<?= $item['id_themes'] ?>" class="btn btn-primary">Modifier</a>
                                                </td>
                                                <td>
                                                    <form action="code.php" method="POST">
                                                        <input type="hidden" name="themes_id" value="<?= $item['id_themes'] ?>" />
